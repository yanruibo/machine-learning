...
'''
生成测试集和训练集
从每个类别的10张图片中随机选取五张作为训练集剩余的五张作为测试集，共40个类别，共200张训练图片，200张测试图片
'''
def generate_data_set():
    train_imagepathes = []
    train_labels = []
    train_indexes = random.sample(range(1, 11), 5)
    test_indexes = list(set(range(1, 11)) - set(train_indexes))
    
    for i in range(1, 41):
        for index in train_indexes:
            # './att_faces/s1/1.pgm'
            train_imagepathes.append('./att_faces/s%s/%s.pgm' % (str(i), str(index)))
    ...
    for i in range(1, 41):
        for index in test_indexes:
            test_imagepathes.append('./att_faces/s%s/%s.pgm' % (str(i), str(index)))
            test_labels.append(i)
    return train_imagepathes, train_labels, test_imagepathes, test_labels
...
def face_identification(n_components=5, L=10, M=4):
    train_imagepathes, train_labels, test_imagepathes, test_labels = generate_data_set()
    # print train_imagepathes
    # print train_labels
...
if __name__ == '__main__':
    ...
    fw = open("log-random6.txt", "a")
    L=14
    #states = 5
    M = 7
    for states in range(5,8):
